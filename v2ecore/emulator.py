...
from v2ecore.output.ae_npz_output import DVSNpzOutput
...
class EventEmulator(object):
    ...
    def __init__(
        ...
        dvs_aedat2, dvs_h5, dvs_text, dvs_npz: str
            names of output data files or None, for npz its folder
        ...
        try:
            ...
            if dvs_npz:
                logger.info('opening text DVS output path ' + dvs_npz)
                self.dvs_npz = DVSNpzOutput(dvs_npz)
    ...
    def generate_events(self, new_frame, t_frame):
        ...
        #--------------------------------------------------
        diff_frame = self.lp_log_frame0 - self.base_log_frame
        ...
        if len(events) > 0:
            ...
            if self.dvs_npz is not None:
                # print('frame_counter, self.event_window_counter-1', self.frame_counter, self.event_window_counter-1, len(events))
                self.dvs_npz.appendEvents(events, self.event_window_counter-1)
