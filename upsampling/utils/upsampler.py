...
class Upsampler:
    ...
    def upsample_sequence(self, sequence: Sequence, dest_imgs_dir: str, dest_timestamps_filepath: str):
        ...
        for img_pair, time_pair in tqdm(next(sequence), total=len(sequence), desc=type(sequence).__name__):
            ...
            for cur_idx, frame in enumerate(total_frames):
                frame = frame[self.crop.iy0:self.crop.iy1,
                                        self.crop.ix0:self.crop.ix1, :]
                self._write_img(frame, idx, dest_imgs_dir)
                if origin_idx == len(sequence) and cur_idx == len(timestamps)-1:
                    continue
                idx += 1
        self._write_timestamps(timestamps_list, dest_timestamps_filepath)
        

    def upsample_sequence_with_events(self, sequence: Sequence, dest_imgs_dir: str, dest_timestamps_filepath: str, dest_events_filepath:str):
        os.makedirs(dest_imgs_dir, exist_ok=True)
        os.makedirs(dest_events_filepath, exist_ok=True)
        timestamps_list = list()

        idx = 0
        origin_idx = 0
        for img_pair, time_pair, events in tqdm(next(sequence), total=len(sequence), desc=type(sequence).__name__):
            origin_idx += 1
            img_pair = self._move_to_device(img_pair, self.device)
            I0 = torch.unsqueeze(img_pair[0], dim=0)
            I1 = torch.unsqueeze(img_pair[1], dim=0)
            ...
            for cur_idx, frame in enumerate(total_frames):
                ...
                if origin_idx == len(sequence) and cur_idx == len(timestamps)-1:
                    continue
                cur_events = self._clip_events(events, [timestamps[cur_idx], timestamps[cur_idx+1] if cur_idx<len(timestamps)-1 else t1])
                self._write_events_npz(cur_events, idx, dest_events_filepath)
                idx += 1

        # timestamps_list += t1
        # self._write_img(img_pair[1], idx, dest_imgs_dir)
        self._write_timestamps(timestamps_list, dest_timestamps_filepath)
        
    def _clip_events(self, events:np.ndarray, timestamps:list):
        t = events[:, 0]
        start = np.where(t>timestamps[0])[0][0]
        end = np.where(t>timestamps[1])[0]
        end = end[0] if len(end) != 0 else len(events)
        # print(start, end, len(events))
    ...
    def _write_img(img: np.ndarray, idx: int, imgs_dir: str):
        ...
        path = os.path.join(imgs_dir, "frame_%010d.png" % idx)
    ...
    def _write_timestamps(timestamps: list, timestamps_filename: str):
        with open(timestamps_filename, 'w') as t_file:
            t_file.writelines([str(idx) +' '+ str(t) + '\n' for idx, t in enumerate(timestamps)])

    @staticmethod
    def _write_events_npz(events:np.ndarray, idx: int, dest_events_filepath:str):
        assert os.path.isdir(dest_events_filepath)
        path = os.path.join(dest_events_filepath, "events_%010d.npz" % idx)
